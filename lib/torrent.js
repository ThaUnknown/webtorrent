...
export default class Torrent extends EventEmitter {
  ...
  addPeer (peer, source) {
    ...
    // connects both tcp and utp peers at once, utp will override tcp if available, otherwise tcp will be used

    const supportsUtp = (this.client.utp && this._isIPv4(host))

    // fails if exists, is invalid or cannot be added
    const wasAddedTcp = !!this._addPeer(peer, 'tcp', source)

    let wasAddedUtp = false
    // won't attempt to add utp peer if tcp peer cant be added
    if (supportsUtp && wasAddedTcp) {
      wasAddedUtp = this._addPeer(peer, 'utp', source)
    }
    if (wasAddedUtp || wasAddedTcp) {
      this.emit('peer', peer)
    } else {
      this.emit('invalidPeer', peer)
    }
    return wasAddedUtp || wasAddedTcp
  }
  ...
  _addPeer (peer, type, source) {
    ...
    if (this._peers[id]) {
      if (type !== 'utp') {
        ...
        this._debug('adding utp peer: duplicate (%s)', id)
      }
    }
    ...
    this._debug('add peer %s and type %s', id, type)
  ...
  removePeer (peerOrID) {
    const id = peerOrID?.id || peerOrID
    const peer = (peerOrID && !peerOrID.id) ? this._peers?.[id] : peerOrID
    ...
    const isUtp = peerOrID?.type === 'utpOutgoing'
    // utp peers are created, without being added to the list, so make sure it's the right type, don't remove tcp peers if utp peer is being removed
    if ((isUtp && this._peers[id].type === 'utpOutgoing') || !isUtp) {
  ...
  _drain () {
    ...
    if (this.client.utp && peer.type === Peer.TYPE_UTP_OUTGOING) {
      ...
      peer.conn.once('connect', () => {
        if (!this.destroyed) {
          // override tcp with utp
          this.removePeer(peer.id)
          this._peers[peer.id] = peer
        } else {
          // normally this is handled in torrent.destroy, but this peer isn't actually ever registered there
          // this means this connection will live for a few seconds past the torrent being destroyed, thats... bad?
          peer.conn.destroy()
        }
      })
    } else {
