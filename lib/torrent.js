...
export default class Torrent extends EventEmitter {
  ...
  addPeer (peer, source) {
    ...
    // add tcp connection, on drain its handled as both tcp and utp
    const wasAdded = !!this._addPeer(peer, 'tcp', source)

    if (wasAdded) {
      this._registerPeer(wasAdded)
      this.emit('peer', peer)
    } else {
      this.emit('invalidPeer', peer)
    }
  ...
  _drain () {
    ...
    if (this.client.utp && peer.type === Peer.TYPE_TCP_OUTGOING) {
      const utpPeer = Peer.createUTPOutgoingPeer(peer, this, this.client.throttleGroups)
      const conn = utp.connect(opts.port, opts.host)
      this.once('close', () => { utpPeer.destroy() })
      this._handleDrainConn(conn, peer, 'utp')
      conn.once('connect', () => {
        if (!this.destroyed) {
          peer.destroy()
          this._registerPeer(utpPeer)
        }
      })
    } else {
      this._handleDrainConn(net.connect(opts), peer, 'tcp')
    }
  }

  _handleDrainConn (conn, peer, type) {
    peer.conn = conn
    peer.startConnectTimeout()
    conn.once('connect', () => { if (!this.destroyed) peer.onConnect(conn) })
